...
function GlobalStyle() {
  return (
    <style>{`
      ...
      .gb-toast-in{ left:50%; animation: gb-toast-in .38s cubic-bezier(.2,.9,.25,1.15); }
...
function ActivationCodeForm({ onActivate, pushToast, accent }) {
  ...
  const submit = () => {
    const result = validateActivationCode(raw);
...
function LoginScreen({ shop, shops, activeShopId, onSwitchShop, vendors, onLogin, pushToast }) {
  ...
  const check = (pin) => {
    if (mode === "admin") {
      if (pin === ADMIN_PIN) { onLogin("admin", "Administrateur"); return true; }
    ...
    <div className="min-h-full flex flex-col items-center justify-center px-6 py-10" style={{ background: "var(--glass)" }}>
      ...
      {!mode ? (
        <div className="w-full max-w-xs flex flex-col gap-3 gb-slide-up">
          ...
          <p className="text-white/30 text-[11px] text-center mt-3 font-mono">Code administrateur (démo) : 1234</p>
        </div>
      ) : (
  ...
  return (
    <div className="rounded-2xl border p-4 mb-3 gb-slide-up" style={{ borderColor: "var(--line)", background: "var(--card)" }}>
      <div className="grid grid-cols-2 gap-2.5">
        ...
        <input className="gb-focus col-span-2 rounded-xl px-3 py-2 text-sm border font-mono" style={{ borderColor: "var(--line)" }} placeholder="Code-barre" value={f.barcode} onChange={(e) => set("barcode", e.target.value)} />
...
function VendorForm({ initial, onSave, onCancel, pushToast, vendors }) {
    ...
    <div className="rounded-2xl border p-4 mb-3 gb-slide-up" style={{ borderColor: "var(--line)", background: "var(--card)" }}>
      ...
      <div className="flex gap-2 mt-3">
        ...
        <button onClick={() => {
          ...
          if (f.pin === ADMIN_PIN || vendors.some((v) => v.pin === f.pin && v.id !== f.id)) { pushToast("Ce code PIN est déjà utilisé", "error"); return; }
...
function ShopForm({ onSave, onCancel, pushToast }) {
  ...
  const submit = () => {
    ...
    onSave(
      { id: uid(), name: name.trim(), type, currency, salesNotificationsEnabled: true, theme: "emeraude", darkMode: false, soundsEnabled: true, joinCode: generateShopJoinCode() },
      { id: uid(), name: vendorName.trim(), pin: vendorPin }
    );
  };
  return (
...
function VendorsSection({ vendors, saveVendors, pushToast }) {
  const [editing, setEditing] = useState(null);
  const [adding, setAdding] = useState(false);
  ...
  return (
    <div>
      ...
      {adding && <VendorForm onSave={upsert} onCancel={() => setAdding(false)} pushToast={pushToast} vendors={vendors} />}
      <div className="flex flex-col gap-2.5">
        {vendors.map((v) => editing === v.id ? (
          <VendorForm key={v.id} initial={v} onSave={upsert} onCancel={() => setEditing(null)} pushToast={pushToast} vendors={vendors} />
        ) : (
...
function AdminScreen({
  ...
  license, licenseStatus, onActivateLicense, onRestoreBackup, pushToast,
}) {
  ...
  return (
    <div className="px-4 pt-4 pb-28">
      ...
      {section === "vendeurs" && <VendorsSection vendors={vendors} saveVendors={saveVendors} pushToast={pushToast} />}
...
export default function App() {
  ...
  const handleActivateLicense = (result) => {
    const now = Date.now();
    const startFrom = license && license.expiresAt && new Date(license.expiresAt).getTime() > now ? new Date(license.expiresAt).getTime() : now;
    const next = result.plan.id === "lifetime"
      ? { code: result.digits, planId: "lifetime", lifetime: true, activatedAt: license?.activatedAt || new Date().toISOString(), expiresAt: null }
      : { code: result.digits, planId: result.plan.id, lifetime: false, activatedAt: license?.activatedAt || new Date().toISOString(), expiresAt: new Date(startFrom + result.plan.days * MS_DAY).toISOString() };
    setLicense(next);
    window.storage.set("license", JSON.stringify(next)).catch(() => {});
    pushToast(result.plan.id === "lifetime" ? "Licence à vie activée !" : `Licence activée — ${result.plan.label}`, "ok");
  };
  ...
  const handleOnboardingComplete = async (shopMeta, vendor) => {
    const newShop = { id: uid(), salesNotificationsEnabled: true, theme: "emeraude", darkMode: false, soundsEnabled: true, ...shopMeta };
              ...
              <>
                ...
                {view === "admin" && role === "admin" && (
                  <AdminScreen
                    ...
                    vendors={vendors} saveVendors={saveVendors} clients={clients} saveClients={saveClients} license={license} licenseStatus={licenseStatus} onActivateLicense={handleActivateLicense} onRestoreBackup={handleRestoreBackup} pushToast={pushToast}
                  />
                )}
