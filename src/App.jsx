...
const OWNER_BIRTHDATE = "1985-11-08"; // format interne d'un <input type="date">
...
function SubscriptionSection({ ownerAccess, onVerifyOwner, pushToast }) {
  ...
  const fetchShops = async (em, bd) => {
    setLoading(true);
    try {
      const data = await api.getOwnerShops({ email: em, birthdate: bd });
  ...
  useEffect(() => {
    if (ownerAccess && allShops === null) {
      fetchShops(ownerAccess.email, ownerAccess.birthdate);
    }
  ...
  const submit = async () => {
    ...
    const ok = await fetchShops(em, birthdate);
    if (ok) {
      onVerifyOwner({ email: em, birthdate });
    ...
    return (
      <div>
        ...
        <div className="rounded-2xl border p-4" style={{ borderColor: "var(--line)", background: "var(--card)" }}>
          ...
          <p className="text-xs opacity-60 mb-4">Renseigne les identifiants propriétaires pour accéder à la vue d'ensemble de <strong>toutes les boutiques, tous appareils confondus</strong>. Vérifié par le serveur — les autres administrateurs n'ont pas accès à cette page.</p>
          ...
          <label className="text-xs font-semibold opacity-60 block mb-1.5">Date de naissance</label>
          <input type="date" value={birthdate} onChange={(e) => setBirthdate(e.target.value)} className="gb-focus w-full rounded-xl px-3 py-3 text-sm border mb-4" style={{ borderColor: "var(--line)", minWidth: 0 }} />
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={() => fetchShops(ownerAccess.email, ownerAccess.birthdate)} disabled={loading} className="gb-focus text-xs font-semibold px-3 py-1.5 rounded-full" style={{ background: "var(--paper-dim)" }}>
          {loading ? "…" : "Actualiser"}
        </button>
      </div>
