...
function OnboardingScreen({ shops, onComplete, onJoinShop, pushToast }) {
  ...
  const submitReconnect = async () => {
    ...
    try {
      ...
      onJoinShop(shopObj, pulled, backend.role, backend.vendorName || "Administrateur");
    } catch (e) {
    ...
    if (!reconnectRole) {
      return (
        <div className="min-h-full flex flex-col items-center justify-center px-6 py-10" style={{ background: "var(--glass)" }}>
          ...
          <button onClick={() => setMode(null)} className="gb-focus mt-7 text-white/50 text-xs underline">Retour</button>
        </div>
      );
    }
    return (
      <div className="min-h-full flex flex-col items-center justify-center px-6 py-10" style={{ background: "var(--glass)" }}>
        ...
        <div className="w-full max-w-xs flex flex-col gap-3">
          <input value={rcShopName} onChange={(e) => setRcShopName(e.target.value)} placeholder="Nom de la boutique" className="gb-focus w-full rounded-2xl px-4 py-3.5 text-sm outline-none" style={{ background: "var(--glass-light)", color: "#fff" }} />
          <input value={rcVendorName} onChange={(e) => setRcVendorName(e.target.value)} placeholder={reconnectRole === "admin" ? "Nom d'un vendeur de ta boutique" : "Ton nom"} className="gb-focus w-full rounded-2xl px-4 py-3.5 text-sm outline-none" style={{ background: "var(--glass-light)", color: "#fff" }} />
          <input value={rcPin} onChange={(e) => setRcPin(e.target.value.replace(/\D/g, "").slice(0, 4))} placeholder={reconnectRole === "admin" ? "Code administrateur" : "Ton code PIN"} inputMode="numeric" className="gb-focus w-full rounded-2xl px-4 py-3.5 text-sm font-mono outline-none" style={{ background: "var(--glass-light)", color: "#fff" }} />
          {reconnectRole === "vendeur" && (
            <input value={rcJoinCode} onChange={(e) => setRcJoinCode(e.target.value.toUpperCase())} placeholder="Ton code de liaison" className="gb-focus w-full rounded-2xl px-4 py-3.5 text-sm font-mono tracking-wider outline-none" style={{ background: "var(--glass-light)", color: "#fff" }} />
          )}
            ...
            <div className="flex-1 min-w-0">
              ...
              {backendLinked ? (
                <div className="mt-0.5">
                  <div className="text-[10px] opacity-40">🔒 Code PIN protégé</div>
                  {v.joinCode && <div className="text-[11px] font-mono font-bold mt-0.5">Code de liaison : {v.joinCode}</div>}
                </div>
              ) : v.pin ? (
                ...
                <div className="text-xs mt-0.5" style={{ color: "var(--danger)" }}>⚠ Aucun code — à réinitialiser</div>
              )}
            </div>
...
export default function App() {
  ...
  const handleOnboardingComplete = async (shopMeta, vendor) => {
    ...
    if (newShop.backendLinked) {
      const keys = ["shopMeta", "vendors", "products", "sales", "categories", "suppliers", "expenses", "movements", "inventories", "clients"];
  ...
  const handleJoinShopComplete = async (shopObj, pulled, autoRole, autoVendorName) => {
