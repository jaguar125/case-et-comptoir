...
const LICENSE_PLANS_PRICING = [
  { id: "1m", name: "Essentiel", duration: "1 mois", price: 6000, oldPrice: 15000 },
  { id: "3m", name: "Avancé", duration: "3 mois", price: 22990, oldPrice: 50000 },
  { id: "6m", name: "Professionnel", duration: "6 mois", price: 39900, oldPrice: 150000 },
  { id: "12m", name: "Entreprise", duration: "12 mois", price: 60600, oldPrice: 250000 },
  { id: "lifetime", name: "Permanent", duration: "À vie", price: null, oldPrice: null },
];

function RegistrationScreen({ onComplete, pushToast }) {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    const record = { name: name.trim(), phone: phone.trim(), email: email.trim(), date: new Date().toISOString() };
    window.storage.set("registeredAdmin", JSON.stringify(record)).catch(() => {});
    api.supportHeartbeat({ name: record.name, phone: record.phone, email: record.email }).catch(() => {});
    setLoading(false);
    onComplete(record);
  };
    ...
    <div className="min-h-full flex flex-col items-center justify-center px-6 py-10" style={{ background: "var(--glass)" }}>
      <div className="mb-8 text-center gb-slide-up">
        <div className="w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center" style={{ background: "var(--cap)" }}>
          <UserPlus size={30} color="var(--glass)" strokeWidth={2.2} />
        </div>
        <h1 className="font-display font-bold text-xl text-white">Bienvenue</h1>
        <p className="text-white/60 text-sm mt-1.5 max-w-[280px] mx-auto">Inscris-toi rapidement avant de configurer ton application.</p>
      </div>
      <div className="w-full max-w-xs flex flex-col gap-3">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ton nom" className="gb-focus w-full rounded-2xl px-4 py-3.5 text-sm outline-none" style={{ background: "var(--glass-light)", color: "#fff" }} />
        <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Numéro de téléphone" className="gb-focus w-full rounded-2xl px-4 py-3.5 text-sm outline-none" style={{ background: "var(--glass-light)", color: "#fff" }} />
        <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email (optionnel)" className="gb-focus w-full rounded-2xl px-4 py-3.5 text-sm outline-none" style={{ background: "var(--glass-light)", color: "#fff" }} />
        <button onClick={submit} disabled={loading} className="gb-focus w-full rounded-2xl py-3 font-semibold text-sm active:scale-[0.98] transition-transform disabled:opacity-50" style={{ background: "var(--cap)", color: "var(--glass)" }}>
          {loading ? "…" : "S'inscrire"}
        </button>
      </div>
    </div>
...
function ActivationScreen({ onActivate, onStartTrial, trialUsed, pushToast, registeredAdmin }) {
  const [showPricing, setShowPricing] = useState(false);
  if (showPricing) {
    return <PricingScreen registeredAdmin={registeredAdmin} onClose={() => setShowPricing(false)} pushToast={pushToast} />;
  }
  return (
    <div className="min-h-full flex flex-col items-center justify-center px-6 py-10 relative" style={{ background: "var(--glass)" }}>
      <button onClick={() => setShowPricing(true)} className="gb-focus absolute top-5 right-5 flex items-center gap-1.5 px-3 py-2 rounded-full text-xs font-semibold text-white" style={{ background: "var(--glass-light)" }}>
        🛒 Acheter une licence
      </button>
      <div className="mb-8 text-center gb-slide-up">
        <div className="w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center" style={{ background: "var(--cap)" }}>
...
function ShopCreatedScreen({ shop, onContinue, pushToast }) {
  const copyCode = async () => {
    try {
      await navigator.clipboard.writeText(shop.joinCode);
      pushToast("Code copié !", "ok");
    } catch { /* presse-papier indisponible */ }
  };
  return (
    <div className="min-h-full flex flex-col items-center justify-center px-6 py-10" style={{ background: "var(--glass)" }}>
      <div className="text-center gb-slide-up mb-8">
        <div className="w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center" style={{ background: "#1CA857" }}>
          <Check size={30} color="#fff" strokeWidth={2.5} />
        </div>
        <h1 className="font-display font-bold text-xl text-white">Boutique créée !</h1>
        <p className="text-white/60 text-sm mt-1.5 max-w-[280px] mx-auto">{shop.name} est prête. Voici le code à partager avec tes futurs vendeurs pour qu'ils rejoignent la boutique depuis leur propre téléphone.</p>
      </div>
      <div className="w-full max-w-xs rounded-2xl p-5 text-center mb-6" style={{ background: "var(--glass-light)" }}>
        <p className="text-white/50 text-[11px] font-semibold uppercase tracking-wide mb-2">Code d'invitation</p>
        <p className="font-display font-bold text-3xl text-white tracking-[0.2em] mb-4">{shop.joinCode}</p>
        <button onClick={copyCode} className="gb-focus w-full rounded-xl py-2.5 text-sm font-semibold flex items-center justify-center gap-1.5" style={{ background: "var(--cap)", color: "var(--glass)" }}>
          <Download size={14} className="rotate-180" /> Copier le code
        </button>
      </div>
      <div className="w-full max-w-xs rounded-2xl p-3.5 mb-6 flex items-start gap-2.5" style={{ background: "var(--glass-light)" }}>
        <ShieldCheck size={14} color="var(--cap)" className="shrink-0 mt-0.5" />
        <p className="text-[11px] text-white/60">Garde ce code confidentiel — quiconque le possède peut se connecter à ta boutique. Tu peux le retrouver à tout moment dans Admin → Boutiques.</p>
      </div>
      <button onClick={onContinue} className="gb-focus w-full max-w-xs rounded-2xl py-3 font-semibold text-sm" style={{ background: "var(--cap)", color: "var(--glass)" }}>
...
function LoginScreen({ shop, shops, activeShopId, onSwitchShop, vendors, onLogin, pushToast }) {
  ...
  const check = (pin) => {
    ...
    let ok = false;
    if (mode === "admin") {
      ok = shop.backendLinked ? api.verifyPin(pin, shop.adminPinHash) : pin === (shop.adminPin || DEFAULT_ADMIN_PIN);
      if (ok) onLogin("admin", "Administrateur");
    } else {
      const v = vendors.find((x) => (shop.backendLinked ? api.verifyPin(pin, x.pinHash) : x.pin === pin));
      if (v) { ok = true; onLogin("vendeur", v.name); }
    }
    if (ok) {
      setFailedAttempts(0);
    } else {
      ...
      if (next >= 5) {
        ...
        setFailedAttempts(0);
        pushToast("Trop de tentatives — réessayez dans 30 secondes", "error");
      } else {
        setFailedAttempts(next);
        pushToast(`Code incorrect (${next}/5)`, "error");
      }
    }
    return ok;
  };
  return (
    <div className="min-h-full flex flex-col items-center justify-center px-6 py-10" style={{ background: "var(--glass)" }}>
      ...
      <div className="mb-10 text-center">
        ...
        <p className="text-white/30 text-[10px] mt-2.5 flex items-center justify-center gap-1"><ShieldCheck size={11} /> Connexion sécurisée</p>
      </div>
      {!mode ? (
        <div className="w-full max-w-xs flex flex-col gap-3 gb-slide-up">
        ...
        <div className="w-full gb-slide-up text-center">
          <div className="w-14 h-14 rounded-2xl mx-auto mb-4 flex items-center justify-center" style={{ background: "var(--danger)" }}>
            <ShieldCheck size={26} color="#fff" />
          </div>
          <p className="text-white font-semibold text-sm mb-1">Accès temporairement bloqué</p>
          <p className="text-white/50 text-xs">Trop de codes incorrects. Réessayez dans <span className="font-mono font-bold text-white/80">{secondsLeft}s</span>.</p>
...
function VendorForm({ initial, onSave, onCancel, pushToast, vendors, adminPin, adminPinHash, backendLinked }) {
  const [f, setF] = useState(initial ? { ...initial, pin: backendLinked ? "" : (initial.pin || "") } : { name: "", pin: "" });
  const set = (k, v) => setF((s) => ({ ...s, [k]: v }));
...
export default function App() {
  ...
  const [registeredAdmin, setRegisteredAdmin] = useState(undefined); // undefined = chargement, null = pas inscrit
  const [justCreatedShop, setJustCreatedShop] = useState(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      let loadedRegisteredAdmin = null;
      try { const r = await window.storage.get("registeredAdmin"); loadedRegisteredAdmin = JSON.parse(r.value); } catch { loadedRegisteredAdmin = null; }
      setLicense(loadedLicense);
      setTrialUsed(!!loadedTrialUsed);
      setOwnerAccess(loadedOwnerAccess);
      setRegisteredAdmin(loadedRegisteredAdmin);
  ...
  const dataReady = license !== undefined && shops !== undefined && registeredAdmin !== undefined && (shops.length === 0 || (products !== null && categories !== null && movements !== null && inventories !== null && clients !== null));
  ...
  return (
    <div className={`gb-root min-h-screen flex justify-center${isDark ? " gb-dark" : ""}`} style={themeVars}>
      ...
      <div className="w-full max-w-[430px] min-h-screen relative" style={{ background: "var(--paper)" }}>
        ...
        {licenseStatus === "none" && registeredAdmin === null && (
          <RegistrationScreen onComplete={(r) => setRegisteredAdmin(r)} pushToast={pushToast} />
        )}
        {licenseStatus === "none" && registeredAdmin && (
          <ActivationScreen onActivate={handleActivateLicense} onStartTrial={handleStartTrial} trialUsed={trialUsed} pushToast={pushToast} registeredAdmin={registeredAdmin} />
        )}
        ...
        {licenseStatus !== "none" && shops.length > 0 && justCreatedShop && (
          <ShopCreatedScreen shop={justCreatedShop} onContinue={() => setJustCreatedShop(null)} pushToast={pushToast} />
        )}

        {licenseStatus !== "none" && shops.length > 0 && shop && !justCreatedShop && (
