...
async function callFunction(name, body) {
  ...
  try {
    ...
    throw new Error(`OFFLINE (${e?.message || "réseau"})`);
  }
...
export async function getShopInfo() {
  return callFunction("get-shop-info", { ...shopAuthFields() });
}
...
export async function flushSyncQueue(getLocalValue) {
  ...
  for (const key of keys) {
    try {
      ...
      if (e.message.startsWith("OFFLINE")) break; // pas la peine d'essayer les autres, pas de réseau
...
export async function syncKeyNow(key, value) {
  try {
...
export async function syncLicenseToShop(license) {
  if (!isShopLinked()) return false;
  return syncKeyNow("shopLicense", license);
}
